...
def algoritmo1(numContributors,numProyects, contributors, projects,skills,usuariosNivel):
   ...
   while (1):
        ...
        #print(proyectoElegido)
        #print(proyectoElegido)
        proyectoElegidoValor=projects[proyectoElegido]
        #print(proyectoElegidoValor)
        for i,skill in  enumerate(proyectoElegidoValor["roles"]):
            
             
             nivel= proyectoElegidoValor["levels"][i]
             usuarios=[clave for clave,valor in  skills[skill]["usuarios"].items() if valor>=nivel and clave not in  usuariosAdd ]
        ...
        if addProject:
            ...
            for i,skill in  enumerate(proyectoElegidoValor["roles"]):
                ...
                array=[usuariosTiempo[usuario1] for usuario1,valor in skills[skill]["usuarios"].items() if valor>=nivel]
        ...
        proyectosTiempo.sort(key=lambda t:projects[t]["daybefore"]+projects[t]["days"]-proyectosTiempoDisponible[t][0],reverse=True)
        #print(proyectosTiempoDisponible[proyectosTiempo[0]][0])
        if proyectosTiempoDisponible[proyectosTiempo[0]][0]<0:
            break
        proyectosPuntuacion.remove(proyectoElegido)
        proyectosPuntuacion1=[t for t  in proyectosPuntuacion if projects[t]["daybefore"]+projects[t]["days"]-proyectosTiempoDisponible[t][0]>0]
...
def algoritmo2(numContributors,numProyects, contributors, projects,skills,usuariosNivel):
   ...
   while (1):
        ...
        for i,skill in  enumerate(proyectoElegidoValor["roles"]):
             ...
             nivel= proyectoElegidoValor["levels"][i]
             usuarios=[clave for clave,valor in  skills[skill]["usuarios"].items() if valor>=nivel and clave not in  usuariosAdd and usuariosTiempo[clave]<tiempo]
             for us in usuariosAdd: 
                
                 if skill in usuariosNivel[us] and skills[skill]["usuarios"][us]==nivel:
                     usuarios=(usuarios)+usuariosNivel[us][skill]
             usuarios=list(set(usuarios)-set(usuariosAdd))
             #usuarios.sort(key=lambda t: skills[skill]["usuarios"][t])
             if (len(usuarios)>0):
                usuario=usuarios[0]
                usuariosAdd.append(usuario)
                usuariosTiempo[usuario]= usuariosTiempo[usuario]+proyectoElegidoValor["days"]
        ...
        proyectosPuntuacion.remove(proyectoElegido)
        #proyectosPuntuacion=list(set(proyectosPuntuacion)-set( proyectosAEliminar))
        #proyectosPuntuacion.sort(key=lambda e:projects[e]["daybefore"]+projects[e]["score"] -projects[e]["days"]- proyectosTiempoDisponible[e][0],reverse=True)
        #proyectosPuntuacion.sort(key=lambda e:projects[e]["score"])
        if len (  proyectosPuntuacion)==0:
            break
