...
def buscar(inicalPos, matriz, R, simbolo, H, W):
    ...
    m2 = inicalPos[1] + 1
    a2 = np.where(matriz[np.ix_(range(max(0, inicalPos[0] - R), inicalPos[0]),
                                range(m2, min(inicalPos[1] + R + 1, W)))] == simbolo)
    a2 = list(a2)
    a2[0] += m1
    a2[1] += m2
    m1 = inicalPos[0] + 1
    ...
    m2 = inicalPos[1] + 1
    a4 = np.where(
        matriz[np.ix_(range(m1, min(inicalPos[0] + R + 1, H)), range(m2, min(inicalPos[1] + R + 1, W)))] == simbolo)
    a4 = list(a4)
    a4[0] += m1
    a4[1] += m2
    m1 = inicalPos[0]
