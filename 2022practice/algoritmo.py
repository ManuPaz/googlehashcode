...
def algoritmo1(dicClientes,ingredientes,cotaSup,operador,minimosINgredientes=None,maximosIngredientes=None):
    ...
    solucion=[]
    while(1):
        #print(len(ingredientesTotales))
        #print(ingredientesTotales)
        ingredientesTotales=funciones.devolverListaOrdenada(ingredientesTotales,ingredientes)
...
def algoritmo2(dicClientes,ingredientes,cotaSup,operador,minimosINgredientes=None,maximosIngredientes=None,numeroInicios=30,aleatorizacionIngredientes=10,numeroAgrupacion=10,iteraciones=100):
   
    ingredientesTotales=list(ingredientes.keys())
    
    #ingredientesTotales=random.sample(ingredientesTotales,numeroInicios)
    
  
    clientesTotales=[]
    clientes=list(dicClientes.keys())
    clientes.sort(key=lambda t:len(dicClientes[t]["si"])+len(dicClientes[t]["no"]))
    #clientes=random.sample(clientes,len(clientes))
    ingredientesAñadidos=[]
    ingredientesProhibidos=[]
  
    for cliente in clientes:
        #print(dicClientes[cliente]["si"])
        if set.intersection(set(dicClientes[cliente]["si"]),set(ingredientesProhibidos))==set():
          if set.intersection(set(dicClientes[cliente]["no"]),set(ingredientesAñadidos))==set():  
             ...
             [ingredientesProhibidos.append(e) for e in dicClientes[cliente]["no"] if e not in ingredientesProhibidos]
             clientesTotales.append(cliente)
             
             
             
    
    [ingredientesAñadidos.append(e) for e,u in ingredientes.items() if set.intersection(set(clientesTotales),set(u["no"]))==set()] 
    [ingredientesProhibidos.append(e) for e,u in ingredientes.items() if set.intersection(set(clientesTotales),set(u["no"]))!=set()]   
    
    ingredientes1=ingredientes
    [ingredientes1[e]["si"].remove(x) for e in ingredientes1.keys() for x in ingredientes1[e]["si"] if x not in clientesTotales]  
    [ingredientes1[e]["no"].remove(x) for e in ingredientes1.keys() for x in ingredientes1[e]["no"] if x not in clientesTotales]
    #print(ingredientes1)
    k=numeroAgrupacion
    ...
    for i in range(iteraciones):
        ...
        if k>0:
            ingredientesProhibidos.sort(key=lambda t:len(ingredientes1[t]["si"])-len(ingredientes1[t]["no"]),reverse=True)
            ingAñadir= ingredientesProhibidos[0:k]
            ingredientesAñadidos.sort(key=lambda t:len(ingredientes1[t]["no"])-len(ingredientes1[t]["si"]),reverse=True)
...
#def devolverAfectados(clientesTotales,diccionarioClientes,ingredientesAñadidos,ingredientesFuera):
...
def devolverIngCliente(x,ingAñadididos,ingFuera,param1,param2):
    ...
    return param1*len(set.difference(set(x["si"]),  ingAñadididos))+param2*len(set.difference(set(x["no"]),  ingFuera))
    
           
def algoritmo3(dicClientes,ingredientes,cotaSup,operador,minimosINgredientes=None,maximosIngredientes=None,numeroInicios=30,aleatorizacionIngredientes=10,numeroAgrupacion=10,iteraciones=100,param1=1,param2=1,salto=100,inicios=10,aleatoriedad=10):
    ...
    ingredientesAñadidos=[]
    ingredientesProhibidos=[]
    clientes.sort(key=lambda t:devolverIngCliente(dicClientes[t], ingredientesAñadidos,ingredientesProhibidos,param1,param2))
    # clientes1=clientes[0:aleatoriedad]
    #clientes1.sort(key=lambda t:devolverIngCliente(dicClientes[t], ingredientesAñadidos,ingredientesProhibidos,param1,param2))
    #print(clientes1)
    #print(clientes)
       ...
       while (len(clientes)>0):
        ...
        if set.intersection(set(dicClientes[cliente]["si"]),set(ingredientesProhibidos))==set():
          if set.intersection(set(dicClientes[cliente]["no"]),set(ingredientesAñadidos))==set():  
             ...
             #print(cliente)
             clientesTotales.append(cliente)
       #print(len(clientesTotales))
       sharedlist[i]=len(clientesTotales)
      
    
    
           
def algoritmo4(dicClientes,ingredientes,cotaSup,operador,minimosINgredientes=None,maximosIngredientes=None,numeroInicios=30,aleatorizacionIngredientes=10,numeroAgrupacion=10,iteraciones=100,param1=1,param2=1,salto=100,inicios=10,aleatoriedad=10):
    ...
    #[shared_list.append(clientesTotales.copy()) for i in range(inicios)]
    process=[]
    for i in range(inicios):
        process.append( multiprocessing.Process(target=ejecutar, args=[i,shared_list,clientes,dicClientes,aleatoriedad,salto,param1,param2]))
...
def algoritmo5(dicClientes,ingredientes,cotaSup,operador,minimosINgredientes=None,maximosIngredientes=None,numeroInicios=30,aleatorizacionIngredientes=10,numeroAgrupacion=10,iteraciones=100,param1=1,param2=1,salto=100,inicios=10,aleatoriedad=10):
    ...
    [ingredientesAñadidos.append(e) for e,u in ingredientes.items() if set.intersection(set(clientesTotales),set(u["no"]))==set() and e not in  ingredientesAñadidos] 
    [ingredientesProhibidos.append(e) for e,u in ingredientes.items() if set.intersection(set(clientesTotales),set(u["no"]))!=set()  and e not in ingredientesProhibidos]   
    ...
    [ingredientesProhidosClientesPos[i].append(e) for i in ingredientesProhibidos for e in list(set.intersection(set(ingredientes[i]["si"]),set(clientesEliminados))) if (not any(k in  ingredientesProhibidos for k in   dicClientes[e]["si"])) and all( k in (ingredientesAñadidos+[i]) for  k in dicClientes[e]["si"])]
    
    [ingredientesProhidosClientesNeg[i].append(e) for i in ingredientesProhibidos for e in list(set.intersection(set(ingredientes[i]["no"]),set(  clientesTotales)))  ]
    
    [ingredientesAñadidosClientesPos[i].append(e) for i in ingredientesAñadidos for e in list(set.intersection(set(ingredientes[i]["si"]),set(clientesTotales)))]
    
    [ingredientesAñadidosClientesNeg[i].append(e)for i in ingredientesAñadidos for e in list(set.intersection(set(ingredientes[i]["no"]),set(  clientesEliminados)))  if (not any(k in  list(set(ingredientesAñadidos)-set((i,))) for k in   dicClientes[e]["no"])) and all( k in (ingredientesAñadidos) for  k in dicClientes[e]["si"]) ]
    
    ingredientesProhibidos.sort(key=lambda t:len(ingredientesProhidosClientesPos[t])-len(ingredientesProhidosClientesNeg[t]),reverse=True)
    ingredientesAñadidos.sort(key=lambda t:len(ingredientesAñadidosClientesPos[t])-len(ingredientesAñadidosClientesNeg[t]))
    while 1:
        if len(ingredientesProhibidos)>0 and len( ingredientesAñadidos)>0:
            ...
            if suma1-suma2>0:
                ...
                #ingredientesProhibidos.append(ing2)
                
                [clientesTotales.append(e) for e in ingredientesProhidosClientesPos[ing1] if e not in ingredientesAñadidosClientesPos[ing2]  ]
                [clientesTotales.append(e) for e in ingredientesAñadidosClientesNeg[ing2] if e not in ingredientesProhidosClientesNeg[ing1]  ]
                [clientesEliminados.remove(e) for e in ingredientesProhidosClientesPos[ing1] if e not in ingredientesAñadidosClientesPos[ing2]  ]
                [clientesEliminados.remove(e) for e in ingredientesAñadidosClientesNeg[ing2] if e not in ingredientesProhidosClientesNeg[ing1]  ]
