...
import numpy as np
def eliminarFoto(foto, fotos, diccionarioTags, diccionarioFotos, diccionarioFotosVerticales,
                 diccionarioFotosHorizontales, tagsPorFoto):
...
def algoritmo1(infogeneral, diccionarioFotos, diccionarioTags, archivo="arch", verticales=True, semilla=1,
               parametroCorte=0.5, paramParada=0.05):
    import random
    random.seed(semilla)
    ...
    if diccionarioFotos[foto1]["vertical"]:
        for k in (diccionarioFotos.keys()):
            ...
            if not diccionarioFotos[foto2]["vertical"]:
                ...
                eliminarFoto(foto2, fotos, diccionarioTags, diccionarioFotos, diccionarioFotosVerticales,
                             diccionarioFotosHorizontales, tagsPorFoto)
                ...
                if len(tagsPorFoto[foto2].intersection(tagsPorFoto[foto1])) == 0:
                    ...
                    eliminarFoto(foto1, fotos, diccionarioTags, diccionarioFotos, diccionarioFotosVerticales,
                                 diccionarioFotosHorizontales, tagsPorFoto)
                    eliminarFoto(foto2, fotos, diccionarioTags, diccionarioFotos, diccionarioFotosVerticales,
                                 diccionarioFotosHorizontales, tagsPorFoto)
                    break;
    ...
    while (len(fotos)) > 0:
        ...
        for i, slic in enumerate([sliceA, sliceB]):
            ...
            if not diccionarioFotos[candidato1]["vertical"]:
                ...
                if len(candidatos2) > 0:
                    ...
                    cont = {t: min([(contador2[t]), suma1 - (contador2[t]), len(tagsAux[t]) - contador[t]]) for t in
                            candidatos2}
                ...
                if candidato2 is None and len(diccionarioFotosVerticales) > 0:
                    ...
                    for foto, e in diccionarioFotosVerticales.items():
                        if foto != candidato1 and len(
                                set.intersection(set(e["tags"]), set(tags1))) <= parametroCorte * len(set(e["tags"])):
                            candidato2 = foto
                            break
                        k += 1
                        if k > parada:
        ...
        for foto in diccionario[u][0]:
            eliminarFoto(foto, fotos, diccionarioTags, diccionarioFotos, diccionarioFotosVerticales,
                         diccionarioFotosHorizontales, tagsPorFoto)
        if u == 0:
...
def algoritmo2(infogeneral, diccionarioFotos, diccionarioTags, archivo="arch", verticales=True, semilla=1,
               parametroCorte=0.5, paramParada=0.05, paramMinimoH=3, paramMaximoH=8, paramMinimoV=2, paramMaximoV=7):
    import random
    random.seed(semilla)
    fotos = [foto for foto in diccionarioFotos.keys()]
    fotos.sort(key=lambda t: len(diccionarioFotos[t]["tags"]))
